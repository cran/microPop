...
getNonBoostFrac=function(microbeNames,resourceNames,numPaths){
    ...
    nonBoostFrac=array(1.0,c(length(microbeNames),length(resourceNames),max(numPaths)),dimnames=list(microbeNames,resourceNames,paste('path',seq(1,max(numPaths)),sep='')))
    
    for (gname in microbeNames){
        data=get(gname)
        nres=colnames(data)
        res=nres[nres!='units' & nres!='Biomass' & nres!='biomass' & nres!='Units']
        
        for (path in 1:numPaths[gname]){
            if (path==1){Rvar='Rtype'}else{Rvar=paste('Rtype.',path,sep='')}
            if (path==1){Bvar='nonBoostFrac'}else{Bvar=paste('nonBoostFrac.',path,sep='')}
            if (any(as.character(data[Rvar,])=='Sb',na.rm=TRUE)){
                booster.name=res[data[Rvar,]=='Sb']
                if (Bvar%in%rownames(data)){
                    nonBoostFrac[gname,booster.name,path]=as.numeric(data[Bvar,1])
                }else{
                    stop(paste('MICROPOP ERROR: There is a boosting resource specified for',gname,'on path',path,'(i.e. ',Rvar,'=Sb for',booster.name,') but no nonBoostFrac specified'))
                }
            }
        }
    }
