...
#' @param varName (string) Name of variable of interest 
...
growthLimFuncDefault=function(strainName,groupName,pathName,varName,resourceValues,allSubType,strainHalfSat,stateVarValues){
    ...
    if (resourceValues[varName]<=0){
        ...
        if (allSubType[varName]=='S' | allSubType[varName]=='Sm'){
            ...
            v=resourceValues[varName]/(strainHalfSat[varName]*(1+sum(resourceValues[allSubType=='S' | allSubType=='Sm']/strainHalfSat[allSubType=='S' | allSubType=='Sm'])))
        }
    }
    return(max(v,0))
}
